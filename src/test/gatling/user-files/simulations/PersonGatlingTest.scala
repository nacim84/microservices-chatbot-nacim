...
class PersonGatlingTest extends Simulation {
    ...
    val scn = scenario("Test the Person entity")
        ...
        .repeat(2) {
            ...
            .get("/microserviceschatbotnacim/api/people")
            .headers(headers_http_authenticated)
            .check(status.is(200)))
            .pause(10 seconds, 20 seconds)
            .exec(http("Create new person")
            .post("/microserviceschatbotnacim/api/people")
            .headers(headers_http_authenticated)
            .body(StringBody("""{"id":null, "firstname":"SAMPLE_TEXT", "lastname":"SAMPLE_TEXT", "birthdate":"2020-01-01T00:00:00.000Z"}""")).asJSON
            .check(status.is(201))
            .check(headerRegex("Location", "(.*)").saveAs("new_person_url"))).exitHereIfFailed
            .pause(10)
            .repeat(5) {
                exec(http("Get created person")
                .get("/microserviceschatbotnacim${new_person_url}")
                .headers(headers_http_authenticated))
                .pause(10)
            }
            .exec(http("Delete created person")
            .delete("/microserviceschatbotnacim${new_person_url}")
            .headers(headers_http_authenticated))
            .pause(10)
        }
